...
public class RecyclerWrapAdapter extends RecyclerView.Adapter implements WrapAdapter {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        } else if (viewType == RecyclerView.INVALID_TYPE - 1 && mFootViews.size() != 0) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        // 的差值来比较，是调用原 Adapter 的 getView 方法，还是获取 footView的 view。
        int numHeaders = getHeadersCount();
        if (position < numHeaders) {
            return;
        }
        int adjPosition = position - numHeaders;
        Log.d("RecyclerWrapAdapter", "adjPosition:" + adjPosition);
        int adapterCount = 0;
        if (mAdapter != null) {
            adapterCount = mAdapter.getItemCount();
            Log.d("RecyclerWrapAdapter", "adapterCount:" + adapterCount);
